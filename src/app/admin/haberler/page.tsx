...
export default async function NewsAdminPage() {
  ...
  return <section className="space-y-8"><div><p className="text-sm font-semibold text-red-600">Duyurular</p><h1 className="text-3xl font-bold">Haber yönetimi</h1></div><NewsForm action={saveNews} />
    <div className="space-y-3"><h2 className="text-xl font-bold">Haberler</h2>{news?.length ? news.map((item) => <details key={item.id} className="rounded-xl border bg-white p-4"><summary className="cursor-pointer font-semibold">{item.title} <span className="text-sm font-normal text-slate-500">{item.status === 'published' ? 'Yayında' : 'Taslak'}</span></summary><div className="mt-4"><NewsForm action={saveNews} item={item} /><form action={deleteNews} className="mt-3 flex gap-2"><input type="hidden" name="id" value={item.id} /><input name="confirmation" required placeholder="DELETE yazın" className={field} /><button className="rounded border border-red-700 px-3 text-red-700">Sil</button></form></div></details>) : <p className="rounded border border-dashed p-6 text-slate-500">Henüz haber yok.</p>}</div></section>
}
