...
export default async function HomePage() {
  ...
  return <div className="min-h-screen overflow-x-clip bg-slate-50 text-slate-900"><Header /><main>
    <section className="relative isolate overflow-hidden bg-slate-950 text-white">
      ...
      <div className="container relative mx-auto px-4 pb-16 pt-14 text-center sm:pb-20 sm:pt-20 lg:pb-24 lg:pt-24">
        ...
        <div className="mx-auto mt-10 grid max-w-4xl grid-cols-2 gap-3 text-left sm:gap-4 lg:grid-cols-4">
    ...
    <section className="relative z-10 -mt-5 border-y border-slate-800 bg-slate-900 text-white shadow-xl sm:-mt-6"><div className="container mx-auto grid grid-cols-2 divide-x divide-slate-700 px-4 sm:grid-cols-4"><ClubFact label="Tamamlanan maç" value={completedMatches} /><ClubFact label="Takım sayısı" value={teams.length} /><ClubFact label="Kadro gücü" value={playerCount} /><ClubFact label="Haber merkezi" value={news.length} /></div></section>
    <section className="container mx-auto grid gap-10 px-4 py-14 lg:grid-cols-2 lg:py-20">
      <ContentSection title="Son haberler" href="/haberler" linkLabel="Tüm haberler">{news.length ? <div className="grid gap-4">{news.map((item) => <article key={item.id} className="group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-red-200 hover:shadow-lg"><p className="text-xs font-bold uppercase tracking-wide text-red-700">{item.category}</p><h3 className="mt-2 text-lg font-bold tracking-tight text-slate-900 group-hover:text-red-800">{item.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-6 text-slate-600">{item.summary}</p></article>)}</div> : <Empty icon={<Newspaper />} text="Henüz yayınlanmış haber yok." />}</ContentSection>
...
function ClubFact({ label, value }: { label: string; value: number }) {
  return <div className="min-w-0 px-3 py-4 text-center sm:px-6"><p className="text-lg font-black sm:text-xl">{value}</p><p className="truncate text-[11px] font-medium text-slate-300 sm:text-xs">{label}</p></div>
}
